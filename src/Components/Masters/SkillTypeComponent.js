...
import { getAllSkillType, addNewSkillType, updateSkillTypeDetails, removeSkillType, } from '../../Redux/Actions/Actions';

function SkillTypeComponent({ skillsTypeListProp, newSkillTypeAddedProp, skillTypeUpdatedProp, skillTypeRemovedProp,
    getAllSkillTypeAction, addNewSkillTypeAction, updateSkillTypeDetailsAction, removeSkillTypeAction, }) {
    ...
    const formik = useFormik({
        ...
        onSubmit: (values, { resetForm }) => {
            if (values?.id) { updateSkillTypeDetailsAction(values) }
            else { addNewSkillTypeAction(values) }
            resetForm();
    ...
    const onSearchfilter = (event) => {
        if (event?.target?.value) { setTableData(skillTypeList.filter(skill => skill.skill_type.toLocaleLowerCase().includes(event.target.value.toLocaleLowerCase()))); }
    ...
    return (
        <>
            <NevbarComponent title={"Skill Types List"} breadcrumbPath={[{ link: 'skilltypes', value: "Skill Types List" }]} />
            <SidebarComponent />
                            ...
                            <form onSubmit={props.handleSubmit}>
                                <div className="modal-body">
                                    <div className="form-group row">
                                        <label htmlFor="skill_type" className="col-sm-2 col-form-label">Skill Type</label>
                                        <div className="col-sm-10">
                                            <input
                                                id="skill_type"
                                                name="skill_type"
                                                type="text"
                                                className={"form-control " + (formik?.errors?.skill_type ? " border-danger " : "") + " "}
                                                onChange={formik.handleChange}
                                                onBlur={formik.handleBlur}
                                                value={formik.values.skill_type}
                                            />
                                            {formik.touched.skill_type && formik.errors.skill_type ? (
                                                <span className="text-danger">{formik.errors.skill_type}</span>
                                            ) : null}
                                        </div>
                                    </div>
                                    <div className="form-group row">
