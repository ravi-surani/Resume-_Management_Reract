...
import { getAllInterviewType, addNewInterviewTypeStatus, updateInterviewTypeDetails, removeInterviewTypeStatus, } from '../../Redux/Actions/Actions';
...
function InterviewTypeComponent({
    getAllInterviewTypeProps, addInterviewTypeProps, updateInterviewTypeProps, removedInterviewTypeProps,
    getAllInterviewTypeAction, addNewInterviewTypeStatusction, updateInterviewTypeDetailsction, removeInterviewTypeStatusction, }) {
    ...
    }, [addInterviewTypeProps, updateInterviewTypeProps, removedInterviewTypeProps,])
    ...
    useEffect(() => {
        if (getAllInterviewTypeProps?.interviewTypesList) {
            let tempList = getAllInterviewTypeProps?.interviewTypesList?.map(interviewType => {
                return {
                    "id": interviewType.id,
                    interview_type: interviewType.interview_type,
                    status: interviewType.status ? <h6 className='text-primary '>Active</h6> : <h6 className='text-danger'>Inactive</h6>,
                    action:
                        <div className="btn-group ">
                            ...
                            <button type="button" className="btn btn-danger btn-sm" onClick={() => onSkillTypeRemove(interviewType.id)}>Remove</button>
    ...
    const onSearchfilter = (event) => {
        if (event?.target?.value) { setTableData(interviewType.filter(modeOfWork => modeOfWork.interview_type.toLocaleLowerCase().includes(event.target.value.toLocaleLowerCase()))); }
        else { setTableData(interviewType); }
    }
    ...
    return (
        <>
            <NevbarComponent title={"Interview Type List"} breadcrumbPath={[{ link: 'modeofwork', value: "Interview Type List" }]} />
            <SidebarComponent />
                            ...
                            <form onSubmit={props.handleSubmit}>
                                <div className="modal-body">
                                    <div className="form-group row">
                                        <label htmlFor="interview_type" className="col-sm-3 col-form-label">Interview Type</label>
                                        <div className="col-sm-9">
                                            <input
                                                id="interview_type"
                                                name="interview_type"
                                                type="text"
                                                className={"form-control " + ((formik?.errors?.interview_type) ? " border-danger " : "") + " "}
                                                onChange={formik.handleChange}
                                                onBlur={formik.handleBlur}
                                                value={formik.values.interview_type}
                                            />
                                            {formik.touched.interview_type && formik.errors.interview_type ? (
                                                <span className="text-danger">{formik.errors.interview_type}</span>
                                            ) : null}
                                        </div>
                                    </div>
...
const mapStatetoProps = (state) => {
    return {
        ...
        addInterviewTypeProps: state?.addInterviewTypeReducer?.newInterviewTypeAdded,
        updateInterviewTypeProps: state?.updateInterviewTypeReducer?.interviewTypeUpdated,
        removedInterviewTypeProps: state?.removedInterviewTypeReducer?.interviewTypeRemoved,
...
const mapDispatchtoProps = {
    ...
    addNewInterviewTypeStatusction: (details) => addNewInterviewTypeStatus(details),
    updateInterviewTypeDetailsction: (details) => updateInterviewTypeDetails(details),
    removeInterviewTypeStatusction: (id) => removeInterviewTypeStatus(id),
}
export default connect(mapStatetoProps, mapDispatchtoProps)(InterviewTypeComponent)