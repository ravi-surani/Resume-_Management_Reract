...
import { getAllUsers, createNewUser, removeUser, } from '../../Redux/Actions/Actions';
...
function UsersComponent({
    userDetailsProp, userListProp, removeUserDetailsProp,
    getAllUsersAction, createNewUserAction, removeUserAction, }) {
        ...
        if (userDetailsProp) {
            let tempList = userDetailsProp.map(user => {
                ...
                return {
                    ...
                    action: <button type="button" className="btn btn-danger btn-sm" onClick={() => onSkillTypeRemove(user.id)}>Remove</button>
    ...
    const onSearchfilter = (event) => {
        if (event?.target?.value) {
            ...
            setTableData(
                userList.filter(
                    usr => usr.user.toLocaleLowerCase().includes(event.target.value.toLocaleLowerCase()) ||
                        usr.email.toLocaleLowerCase().includes(event.target.value.toLocaleLowerCase()) ||
                        usr.contect_no.toLocaleLowerCase().includes(event.target.value.toLocaleLowerCase()))
            );
        }
        else { setTableData(userList); }
    }
    ...
    return (
        <>
            <NevbarComponent title={"Users List"} breadcrumbPath={[{ link: 'Users', value: "Users List" }]} />
            <SidebarComponent />
                                    ...
                                    <div className="form-group row">
                                        ...
                                        <div className="col-sm-9">
                                            <input
                                                ...
                                                name="password"
                                                type="password"
                                                className={"form-control " + ((formik?.errors.password) ? " border-danger " : "") + " "}
                                                onChange={formik.handleChange}
                                                onBlur={formik.handleBlur}
