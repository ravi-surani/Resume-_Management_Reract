...
import { getAllInterviewMode, addNewInterviewMode, updateInterviewModeDetails, removeInterviewModeStatus, } from '../../Redux/Actions/Actions';
...
function InterviewModeComponent({
    interviewModeListProp, newinterviewModeAddedProp, interviewModeUpdatedProp, interviewModeRemovedProp,
    getAllInterviewModeAction, addNewInterviewModeAction, updateInterviewModeDetailsAction, removeInterviewModeStatusAction, }) {
    ...
    }, [newinterviewModeAddedProp, interviewModeUpdatedProp, interviewModeRemovedProp,])
        ...
        if (interviewModeListProp) {
            let tempList = interviewModeListProp.map(interviewMode => {
                return {
                    ...
                    status: interviewMode.status ? <h6 className='text-primary '>Active</h6> : <h6 className='text-danger'>Inactive</h6>,
                    action:
                        <div className="btn-group ">
                            ...
                            <button type="button" className="btn btn-danger btn-sm" onClick={() => onSkillTypeRemove(interviewMode.id)}>Remove</button>
    ...
    const onSearchfilter = (event) => {
        if (event?.target?.value) {
            setTableData(interviewModeList.filter(interview_mode => interview_mode.interview_mode.toLocaleLowerCase().includes(event.target.value.toLocaleLowerCase())));
        }
        else { setTableData(interviewModeList); }
    }
    ...
    return (
        <>
            <NevbarComponent title={"Interview Mode"} breadcrumbPath={[{ link: 'interviewmode', value: "Interview Mode" }]} />
            <SidebarComponent />
                    ...
                    <div className="content-header row">
                        ...
                        <div className="form-group col-6">
                            <div className="input-group">
                                ...
                                <div className="input-group-append">
                                    <button className="btn btn-primary" onClick={() => { setIsModelOpen(true) }}>Add</button>
                                </div>
                            </div>
                        </div>
                    </div>
                            ...
                            <form onSubmit={props.handleSubmit}>
                                <div className="modal-body">
                                    <div className="form-group row">
                                        <label htmlFor="interview_mode" className="col-sm-3 col-form-label">Interviewer</label>
                                        <div className="col-sm-9">
                                            <input
                                                id="interview_mode"
                                                name="interview_mode"
                                                type="text"
                                                className={"form-control " + ((formik?.errors.interview_mode) ? " border-danger " : "") + " "}
                                                onChange={formik.handleChange}
                                                onBlur={formik.handleBlur}
                                                value={formik.values.interview_mode}
                                            />
                                            {formik.touched.interview_mode && formik.errors.interview_mode ? (
                                                <span className="text-danger">{formik.errors.interview_mode}</span>
                                            ) : null}
                                        </div>
                                    </div>
                                    <div className="form-group row">
...
const mapStatetoProps = (state) => {
    return {
        ...
        newinterviewModeAddedProp: state?.addInterviewModeReducer?.newinterviewModeAdded,
        interviewModeUpdatedProp: state?.updateInterviewModeReducer?.interviewModeUpdated,
        interviewModeRemovedProp: state?.removedInterviewModeReducer?.interviewModeRemoved,
...
const mapDispatchtoProps = {
    ...
    updateInterviewModeDetailsAction: (details) => updateInterviewModeDetails(details),
    removeInterviewModeStatusAction: (id) => removeInterviewModeStatus(id),
}
export default connect(mapStatetoProps, mapDispatchtoProps)(InterviewModeComponent)