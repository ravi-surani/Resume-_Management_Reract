...
import { getAllSource, addNewSource, updateSourceDetails, removeSource, } from '../../Redux/Actions/Actions';

function SourceComponent({ sourceListProp, newSourceAddedProp, sourceUpdatedProp, sourceRemovedProp,
    getAllSourceAction, addNewSourceAction, updateSourceDetailsAction, removeSourceAction, }) {
    ...
    const formik = useFormik({
        ...
        onSubmit: (values, { resetForm }) => {
            if (values?.id) { updateSourceDetailsAction(values) }
            else { addNewSourceAction(values) }
            resetForm();
        ...
        if (sourceListProp) {
            let tempList = sourceListProp.map(source => {
                return {
                    ...
                    status: source.status ? <h6 className='text-primary '>Active</h6> : <h6 className='text-danger'>Inactive</h6>,
                    action:
                        <div className="btn-group ">
                            ...
                            <button type="button" className="btn btn-danger btn-sm" onClick={() => onSourceRemove(source.id)}>Remove</button>
                        </div>
                }
            })
    ...
    const onSearchfilter = (event) => {
        if (event?.target?.value) { setTableData(sourceList.filter(source => source.source_name.toLocaleLowerCase().includes(event.target.value.toLocaleLowerCase()))); }
        else { setTableData(sourceList); }
    }
    ...
    return (
        ...
        <>
            ...
            <NevbarComponent title={"Source List"} breadcrumbPath={[{ link: 'source', value: "Source List" }]} />
            <SidebarComponent />
                        ...
                        <div className="form-group col-6">
                            <div className="input-group">
                                <div className="custom-file">
                                    <input type="text" className="form-control" ref={searchRef} name='search' placeholder="Search " onChange={onSearchfilter} />
                                </div>
                                    ...
                                    <div className="form-group row">
                                        ...
                                        <div className="col-sm-10">
                                            <input
                                                ...
                                                name="source"
                                                type="text"
                                                className={"form-control " + (formik?.errors?.source ? " border-danger " : "") + " "}
