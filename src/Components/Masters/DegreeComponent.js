...
import { getAllDegree, addNewDegree
, updateDegreeDetails, removeDegreeStatus, } from '../../Redux/Actions/Actions';
...
function DegreeComponent({
    DegreeListProp, newDegreeAddedProp, DegreeUpdatedProp, DegreeRemovedProp,
    getAllDegreeAction, addNewDegreeModeAction, updateDegreeModeDetailsAction, removeDegreeModeStatusAction, }) {
        ...
        if (DegreeListProp) {
            let tempList = DegreeListProp.map(degree => {
                return {
                    ...
                    status: degree.status ? <h6 className='text-primary '>Active</h6> : <h6 className='text-danger'>Inactive</h6>,
                    action:
                        <div className="btn-group ">
                            ...
                            <button type="button" className="btn btn-danger btn-sm" onClick={() => onSkillTypeRemove(degree.id)}>Remove</button>
    ...
    return (
        <>
            <NevbarComponent title={"Degree List"} breadcrumbPath={[{ link: 'interviewer', value: "Degree List" }]} />
            <SidebarComponent />
                            ...
                            <form onSubmit={props.handleSubmit}>
                                <div className="modal-body">
                                    <div className="form-group row">
                                        <label htmlFor="degree" className="col-sm-3 col-form-label">Degree</label>
                                        <div className="col-sm-9">
                                            <input
                                                id="degree"
                                                name="degree"
                                                type="text"
                                                className={"form-control " + ((formik?.errors.degree) ? " border-danger " : "") + " "}
                                                onChange={formik.handleChange}
                                                onBlur={formik.handleBlur}
                                                value={formik.values.degree}
                                            />
                                            {formik.touched.degree && formik.errors.degree ? (
                                                <span className="text-danger">{formik.errors.degree}</span>
                                            ) : null}
                                        </div>
                                    </div>
...
const mapDispatchtoProps = {
    ...
    addNewDegreeModeAction: (details) => addNewDegree
(details),
